...
const DIMS = [
  ...
  { key: 'flags', col: 5, label: '属性・イベント', agg: '属性を問わず', bits: true, def: '大学発＝経産省 大学発ベンチャーDB掲載。他社を吸収＝合併の承継先になった会社。上場した＝新規公開時の有価証券届出書か証券コード付き有価証券報告書あり。TOB＝公開買付届出書の対象（2021年9月以降）。' },
...
function catFor(d, id) {
  ...
  else if (d.key === 'flags') c = [find('attr', '大学発ベンチャー'), find('exit', '他社を吸収した（合併の承継先）'), find('exit', '上場した'), find('exit', 'TOB（公開買付）の対象になった')][id];
