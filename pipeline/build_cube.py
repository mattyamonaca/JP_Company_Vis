...
行: [設立年idx, 都道府県idx, 法人種別idx, 出口idx(0 なし,1 清算,2 吸収合併,3 登記官,4 その他,5 上場中), 閉鎖年idx, フラグ(bit0 大学発, bit1 他社を吸収), 商号KWビット, EDINET業種idx, 技術分野idx, 社数]
...
STATUS = [('A', '存続（休眠含む）'), ('L', '清算結了'), ('M', '吸収合併された'), ('R', '登記官による閉鎖'), ('O', 'その他の閉鎖'), ('P', '上場している'), ('Q', '上場したが現在は非上場')]
...
scope = {hb: (l or 0, u or 0, i or 0, t or 0, q or 0, w or 0) for hb, l, u, i, t, q, w in con.execute("SELECT houjin_bangou, is_listed, is_univ_startup, has_ipo_filing, is_tob_target, acquired_by_listed, acquired_web FROM company_scope")}
...
for hb, a, pref, kind, closed, reason in con.execute("""SELECT houjin_bangou, assigned_on, pref_code, kind_code, closed_on, close_reason_code
    ...
    l, u, ip, tb, aq, wb = scope.get(hb, (0, 0, 0, 0, 0, 0)); flags = (1 if u else 0) | (2 if hb in acquirers else 0) | (4 if ip else 0) | (8 if tb else 0) | (16 if aq else 0) | (32 if wb else 0)
...
obj = {'total': n, 'dims': {
    ...
    'flags': ['大学発ベンチャー', '他社を吸収した（合併の承継先）', '上場した（新規公開の届出あり）', 'TOB（公開買付）の対象になった', '上場企業に買収された（株式取得）', '買収・子会社化の報道あり'], 'kw': [k[1] for k in kw], 'jpx': ['（上場企業以外）'] + jpx, 'field': ['（大学発以外）'] + fld},
    'rows': rows}
